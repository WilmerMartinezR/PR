...
#             (MHprices)
######################################################################
source("/Users/wilmermartinez/Documents/RA_Jev/R_program/ReadFilesSuicides.R")

# This unit 72049 does not have info for Suicides, then it is removed
Suicides4 <- Suicides4 %>%
  filter(GEO_ID != 72049)

library("writexl")
file.data = "/Users/wilmermartinez/Documents/RA_Jev/data/Final_data/"
nom_out2 <- paste(file.data, "Suicicdes.xlsx",sep = "")
...
Full_list_variables <- "D + Time + HI+ Cognitive+y5_17 +
                     y18_34+y35_64+y65_74+Unemp+Male+IncomePC"

Full_list_variables_wD <- "Time + HI+ Cognitive+y5_17 +
                     y18_34+y35_64+y65_74+Unemp+Male+IncomePC"

# counter effet variable name
CountFE_name = "SuicidesH0"
...
# Result
# outputUnitChang
#chang_S <- outputUnitChang$model_ChangU$ModelFit
#chang_S <- chang_S[-c(1:9),c("GEO_ID","SigPval")]

chang_S <- model_ChangU1$ModelFit # interact D
...
plot(chang_MHP$SigPval[-c(1:9)], chang_MHP$Estimate[-c(1:9)], xlab="Significant",
     ylab="", main="Hurricane Effect")
abline(h=0,col="red")
plot(chang_MHP2$SigPval[-c(1:9)], chang_MHP2$Estimate[-c(1:9)], xlab="Significant",
     ylab="", main="y5_17 Effect")
abline(h=0,col="red")
plot(chang_MHP3$SigPval[-c(1:9)], chang_MHP3$Estimate[-c(1:9)], xlab="Significant",
     ylab="", main="Income Effect")
abline(h=0,col="red")
plot(chang_MHP4$SigPval[-c(1:9)], chang_MHP4$Estimate[-c(1:9)], xlab="Significant",
...
chang_MHP_all2 <- chang_MHP_all[-c(1:9),]  
...
Name_output <- "SuicidesSig_ModNewB"
...
for(w in 0:1){
  ...
  box_summary(datos1a[,-c(ncol(datos1a))], paste(titles[w+1], proport," (%)",sep=""),
              xfin=6, yini=Start_year,yfin=Last_year,xint=4.7,
...
# To save this plot define
save1 = 1 # save, or = 0 just to print 
Name_output2 <- "SuicidesUnits_ModNewB"
file.out.ex = "/Users/wilmermartinez/Documents/RA_Jev/R_program/Simu_result_ts_interv/"
format = "eps"
#format = "png"
...
stargazer(model1,model2, model3, model4,
          ...
          type = "latex", 
          se = rob_se,
          title = "Linear Panel Regression Models of Suicides Rate",
          model.numbers = FALSE,
...
write.csv(ToMap, file = paste(file.out.ex, "SucidesCases",".csv",sep=""), row.names = FALSE)
...
Twoy <- TreatE(datFore2, "Dif_Suicides")
Y2017 <- TreatE(datFore2a, "Dif_Suicides")
Y2018 <- TreatE(datFore2b, "Dif_Suicides")

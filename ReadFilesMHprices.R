...
MedianH <- read.table(paste(files.ready, "Median_Phouses.txt",sep=""), header = T)
...
Suicides <- MedianH
Suicides$GEO_ID2 <- substr(Suicides$GEO_ID2,10,14)
colnames(Suicides)[1] <- "GEO_ID"
Suicides <- Suicides[,-c(ncol(Suicides))]
Suicides <- merge(Suicides, SuicidesA[,1:2],by.x = "GEO_ID",by.y = "GEO.id2")
###########################################################################
...
Suicides2 <- longform(Suicides, "^MH", "MedianH")
...
#Suicides3 <- merge(Suicides2, Cognitive2[,c(2,4)], by.x = "Merge", by.y = "Merge")
